...
* @Last Modified time: 2016-07-26 18:48:21
*/
			...
			for(var r=0;r<game.cbm.instance.length;r++){
				for(var c=0;c<game.cbm.instance[r].length;c++){
					if(game.cbm.instance[c][r]!=null){
						if(game.cbm.instance[c][r].col<this.w){
					...
					if(game.cbm.instance[c][length-r]!=null){
						if(game.cbm.instance[c][length-r].col+this.w>=game.canvas.width){
			...
			for(var i=0;i<game.cbm.instance.length;i++){
				for(var j=0;j<game.cbm.instance[i].length;j++){
					if(game.cbm.instance[i][j]!=null){
						var row=game.cbm.instance[i][j].row/this.h;
						var col=game.cbm.instance[i][j].col/this.w;
						if(!isNo){
							...
							for(var r=0;r<game.cbm.instance.length;r++){
								for(var c=0;c<game.cbm.instance[r].length;c++){
									if(game.cbm.instance[r][c]!=null){
										this.map[game.cbm.instance[r][c].row/this.h][game.cbm.instance[r][c].col/this.w]=game.cbm.instance[r][c].c+1;
									}
								}
							}
							game.cbm.row=0;
